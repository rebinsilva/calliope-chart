    ...
    _createClass(IsotypeCluster, [{
        ...
        value: function displayDistribution() {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var _ret = function() {
                    ...
                    var typesizey = svg.select('#pictypeicdis' + pictype).node().getBBox().height; /*---------------计算每个bar需要的icon数量---------------------*/
                    var xpadding = 2;
                    var ypadding = 2;
                    var total = void 0;
                    ...
                    initYAixsStyle(startY, endY, maxY, svg, axisWidth, tickSize, margin); /*-------------------在图表中添加icon---------------------------------*/
                    var rowa = [];
                    scale1 = scale;
                    ...
                    var isRotate = false;
                    var xAxis = svg.append("g").attr("class", 'xAxis').attr("transform", 'translate(0,' + (height + offsetY) + ')'); //间隔显示ticks 
        ...
        value: function displayCategorization() {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var _ret3 = function() {
                    ...
                    var typesizey = svg.select('#pictypeiccate' + pictype).node().getBBox().height; /*---------------计算每个bar需要的icon数量---------------------*/
                    var xpadding = 2;
                    var ypadding = 2;
                    var total = void 0;
                    if (chartsize === _size2.default.LARGE) {
        ...
        value: function displayValue() {
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var typesizey1 = svg.select('#pictypeicv' + pictype).node().getBBox().height; /*-------------根据chartsize的数据来进行icon的缩放------------------*/
                var scale = void 0;
                var scaley = 20;
                var xpadding = 2;
                var ypadding = 2;
                ...
                var rowa = Math.ceil(valuePict / 4); /*----------------计算缩放后的icon长宽------------------------*/
                ...
                y = d3.scaleLinear().nice().range([height - offsetY, height - typesizey - offsetY]).domain([0, barValue]).clamp(true); /*-------------------在图表中添加icon---------------------------------*/
                var scale1 = scale;
                ...
                var toolTipX = width / 2, //箭头中心所在x的位置
                    toolTipY = y(0) - rowa * typesizey * scale1 - offsetY * 2 - 20 * chartSize.height / 320, //箭头中心所在y的位置 ,原先是40*
        ...
        value: function displayRank() {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var _ret5 = function() {
                    ...
                    var typesizey = svg.select('#pictypeicr' + pictype).node().getBBox().height; /*---------------计算每个bar需要的icon数量---------------------*/
                    var xpadding = 2;
                    var ypadding = 2;
                    var total = void 0;
                    if (chartsize === _size2.default.LARGE) {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var _ret7 = function() {
                    var _getSizeBySize5 = getSizeBySize(_this6, "difference"),
                        ...
                        width = chartSize.width - margin.left - margin.right,
                        height = chartSize.height - margin.top - margin.bottom;
                    var svg = d3.select(_this6.container()).append("svg").attr("width", chartSize.width).attr("height", chartSize.height).append("g").attr("id", "isotypeclusterDifference").attr("transform", 'translate(' + margin.left + ',' + margin.top + ')'); //difference类型会在filteredData里生成两条数据
                    ...
                    var typey = svg.select('#pictypeicdiff' + pictype).node().getBBox().y; // /*------------------在图表中添加icon----------------------------*/
