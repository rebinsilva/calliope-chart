    ...
    _createClass(IsotypeProportion, [{
        ...
        value: function displayDifference() {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                var _ret = function() {
                    ...
                    })).range([height - 40 * chartSize.height / 320, 0]).padding(0.6); /*-------------根据x的数据来进行icon的缩放--------------------------------*/
                    var scalex = void 0;
                    var scaley = void 0;
                    ...
                    if (_this3.size() !== _size2.default.SMALL) {
                        ...
                        var toolTipX = startPos; //箭头中心所在x的位置
        ...
        value: function displayProportion() {
            ...
            if (this.style() === _style2.default.PICTOGRAPH) {
                ...
                var data = filteredData; /*------------------通过名称找寻icon----------------------------*/
                ...
                svg.select('#pictypeisopro' + pictype).attr("transform", function() {
                    ...
                    scaley = scalex;
                    return 'scale(' + scalex + ',' + scaley + ')';
                }); /*----------------计算缩放后的icon长宽------------------------*/ // let typesizex=svg.select(`#pictype${pictype}`).node().getBoundingClientRect().width;
                ...
                var typey = svg.select('#pictypeisopro' + pictype).node().getBBox().y; /*----------------计算百分比--------------------------*/
                ...
                svg.append("g").append("text").attr("x", 0).attr("y", height / 2 - typesizey / 2 - ybuffer).attr("text-anchor", "end").attr("alignment-baseline", "middle").attr('font-family', TEXTFONT).attr("font-size", annotationSize).attr("font-weight", 600).attr("fill", _color2.default.HIGHLIGHT).text(focus[0].value); //使图表居中
                var cardWidth = chartSize.width;
                var cardHeight = chartSize.height;
                var a = svg.node().getBBox().width;
                var b = svg.node().getBBox().height;
